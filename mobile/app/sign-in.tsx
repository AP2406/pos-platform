  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.topBar}>
        {/* THE REAL LOCKUP REPLACES THE TYPESET PILL. The native app's one
            unambiguous brand moment is this sign-in screen, so it gets the
            horizontal lockup at the kit's 220pt floor, not "SURGE"
            letterspaced inside a blue chip. That chip was also the worst
            white-on-blue case on the surface: at 14pt it needed 4.5:1 and the
            kit blue gives 3.39:1, so replacing it removes the contrast problem
            ...
            MERGE NOTE: the lockup landed on a centred, vertically stacked
            brand block; the PIN-pad pass moved that block into this top bar
            and put the screen's weight on the device role instead. Both hold:
            the lockup keeps its kit geometry, the row keeps its layout. */}
...
const styles = StyleSheet.create({
  ...
  // its native 980:290 ratio — no stretching. `mark`/`markTxt`, the blue chip
  // this replaced, are gone with it: the Image is the only consumer now.
