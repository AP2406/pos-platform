-- P0-10 money pass. Additive. Two distinct money concepts that must stay
-- separate on the immutable snapshot:
--   void_total    — value of items VOIDED (not made); excluded from revenue
--   auto_gratuity — a mandatory large-party gratuity (taxable in Canada),
--                   recorded apart from manual service_charge and from tip
-- These persist via create_pos_order (recompute happens in the server action;
-- the RPC stores the client-computed values, like comp/service_charge).

alter table public.orders
  add column if not exists void_total numeric not null default 0,
  add column if not exists auto_gratuity numeric not null default 0;
...
begin
  ...
  insert into public.orders (
    business_id, status, subtotal, tax, tip, discount, comp, service_charge,
    void_total, auto_gratuity, total,
    ...
    coalesce((payload->>'comp')::numeric,0), coalesce((payload->>'service_charge')::numeric,0),
    coalesce((payload->>'void_total')::numeric,0), coalesce((payload->>'auto_gratuity')::numeric,0), v_total,
