// Dependency-free QR Code generator (byte mode), vendored so nothing calls out to
// an external CDN or npm package. Faithful port of Project Nayuki's "QR Code
// generator" reference (MIT License), trimmed to what we need: encode arbitrary
// text (UTF-8, byte segment), auto version + auto mask, and render to an SVG path.
// Used by the "Print table QR codes" page to turn each table's pay-at-table URL
// into a scannable code. No secrets, no network, no state — a pure function.
//
// Reference: https://www.nayuki.io/page/qr-code-generator-library

export type Ecc = "L" | "M" | "Q" | "H";

// Error-correction level -> its ordinal (row into the tables) + format bits.
...
// [ecl ordinal][version] — codewords per ECC block. Column 0 is an unused placeholder.
...
// [ecl ordinal][version] — number of ECC blocks. Column 0 is an unused placeholder.
...
// Byte mode: 8 char-count bits for v1-9, else 16.
...
export class QrCode {
  ...
  static encodeText(text: string, ecl: Ecc = "M"): QrCode {
    const bytes = Array.from(new TextEncoder().encode(text));
    return QrCode.encodeBytes(bytes, ecl);
  }

  static encodeBytes(bytes: number[], ecl: Ecc): QrCode {
    // Smallest version that fits this byte segment at the given ECC level.
    ...
    appendBits(bytes.length, numCharCountBits(version), bb);
  ...
  private constructor(
    ...
    // Auto-select the mask with the lowest penalty.
  ...
  private drawFunctionPatterns(): void {
    ...
    this.drawFormatBits(0); // placeholder, re-drawn after masking
    this.drawVersion();
  }
  ...
  private finderPenaltyTerminateAndCount(currentRunColor: boolean, currentRunLength: number, rh: number[]): number {
  ...
  private finderPenaltyAddHistory(currentRunLength: number, rh: number[]): void {
    let len = currentRunLength;
...
export type QrSvgOptions = {
  ecc?: Ecc;
  border?: number; // quiet-zone modules
  dark?: string;
  light?: string;
};

// Render text as a self-contained SVG string (crisp, scalable, print-friendly).
