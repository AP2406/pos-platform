...
// Daily cron (vercel.json — Hobby plans allow one run/day): email a reminder for
// upcoming bookings that have an email and haven't been reminded, when they fall
// within the business's reminder window (settings.reservation_reminder_hours,
// default 24h ≈ a day-before/morning-of reminder). Best-effort + idempotent via
// reminded_at. Protected by CRON_SECRET when configured.
export async function GET(request: Request) {
  ...
  const horizonIso = new Date(now + 48 * 3600000).toISOString(); // generous upper bound; per-business window applied below
  ...
  for (const r of rows ?? []) {
    ...
    const windowH = Number.isFinite(hours) && hours > 0 ? hours : 24;
