  ...
  return (
    <div>
      ...
      {rows.length === 0 ? (
        ...
        <>
          {/* Amber matches the Inventory page this report is reached from,
              which already colours "N at/below reorder" that way. Red is added
              for out-of-stock, a harder state the parent page has no concept of
              — nothing on hand at all, versus merely under the line. */}
                ...
                {rows.map((r, idx) => {
                  ...
                  return (
                    <tr key={r.kind + idx} className="border-b border-border last:border-0">
                      {/* No per-row marker, and that is NOT an inconsistency
                          with the Inventory page's ⚠. There, the table lists
                          every tracked ingredient and the mark picks out the
                          low subset. Here every row is low by definition, so a
                          mark on all of them would carry no information — and
                          out-of-stock is already said by the on-hand figure
                          reading 0 in red. */}
                      <td className="py-2 px-4 font-medium">{r.name}</td>
                      <td className="py-2 px-4 text-muted-foreground">{r.kind}</td>
                      <td className={"py-2 px-4 text-right tabular-nums " + (out ? "text-red-500 font-medium" : "")}>
