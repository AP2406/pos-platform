...
const OT_HOURS = 44; // Ontario weekly OT threshold (used as a 7-day flag)

type Row = { id: string; name: string; hours: number; rate: number | null; cost: number; sales: number };
...
export default async function LaborPage({
  ...
  const rows = Array.from(byId.values())
    .map((r) => ({ ...r, hours: Math.round(r.hours * 100) / 100, cost: Math.round((r.rate ? r.hours * r.rate : 0) * 100) / 100, sales: Math.round(r.sales * 100) / 100 }))
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        ...
        <Stat label="Hours" value={(Math.round(totHours * 10) / 10).toFixed(1)} />
      </div>
        ...
        <div className="bg-card ring-1 ring-line shadow-elevation rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((r) => {
                  const ot = range === "7d" && r.hours > OT_HOURS;
                  ...
                  return (
                    <tr key={r.id} className="border-b border-border last:border-0">
                      <td className="px-3 py-2 font-medium">
                        {r.name}
                        {ot && <span className="ml-2 text-xs text-amber-600 font-semibold">OT</span>}
