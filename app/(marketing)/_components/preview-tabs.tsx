...
export function PreviewTabs({ tabs, label, aside }: { tabs: PreviewTab[]; label: string; aside?: ReactNode }) {
  ...
  return (
    <div className="grid items-start gap-[var(--surge-space-7)] lg:grid-cols-[minmax(0,40%)_minmax(0,1fr)] lg:gap-[var(--surge-space-8)]">
      <div>
      {aside}
      <div role="tablist" aria-label={label} className="mt-[var(--surge-space-7)] flex flex-wrap gap-[var(--surge-space-6)] border-b border-[var(--surge-on-dark-line)]">
        {tabs.map((t, i) => {
