...
| **Item rows** | | | |
| Thumbnail photo | **Yes** | `catalog_items.image_url`; real upload path to the public `item-images` storage bucket, policies in `0001_register_photos_categories.sql:31-45` | None. An item with **no** photo shows its register tile colour (`tileClassesFor()`), never a logo; an item whose stored URL no longer resolves falls back to the same swatch on `onError` rather than to the browser's broken-image glyph. |
...
| **Top bar** | | | |
| Location switcher (business + sub-label) | **Yes, in the sidebar** | `WorkspaceSwitcher` (`app/app/_components/app-shell.tsx`) — name + mode + role, plus roll-up links | None. **Not duplicated into the top bar**; two switchers on one screen is a bug, not a feature. The top bar carries a breadcrumb in that slot instead. |
| User chip (name + role) | **Yes** | `TopBar` (`app/app/_components/top-bar.tsx`); name from Supabase `user_metadata.full_name` falling back to the sign-in address, role label from `WEB_ROLE_LABELS` | None. Sign-out and the theme switch stay at the foot of the sidebar, which is the only copy the phone drawer can reach. |
| `Search anything` field | **No global search exists** | — | **Omitted as specified, replaced with something real.** There is no index over orders, items, guests or staff, so a "search anything" field would be a promise nothing keeps. The slot carries a **Jump to** palette (⌘K) over this viewer's own nav destinations — which is exactly what a nested rail needs. |
| `Systems operational` dot | **Reworded to something checkable** | `ConnectionDot` in `top-bar.tsx` — `navigator.onLine` + the online/offline events | Small. We have no status endpoint, so the dot reports **this browser's** connectivity and says "Connected" / "Offline" rather than making a claim about our infrastructure. |
| **Sidebar** | | | |
| `Overview, Orders, Floor plan, Menu builder, Inventory, Team, Reports, Settings, Help & support` (9 items) | **Restructured to match the mockup's SHAPE, not its labels** | `lib/modules/nav.ts` | **Was 37 flat links under 6 headings; now 8 primaries with the long tail folded underneath.** Hardcoding the mockup's nine labels would still break the permission model and every non-restaurant vertical, so the labels stay mode-derived — but the *count* is now inside the category norm of 10–16 (`docs/competitor-dashboard-study.md` §1.1). Nothing was removed: a child only nests under a parent that survived the same mode + permission filter, and is promoted to the top level otherwise. `presets.ts:47` still varies this module's word per vertical (`Menu` / `Products` / `Services` / `Items`), and the page title and the nav link both read it, so the two can't disagree. |
